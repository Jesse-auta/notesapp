...
const AiChatPanel = ({ onClose, notes }) => {
    ...
    const handleSend = async (text) => {
        ...
        try {
            // Search notes locally by keyword match
    ...
    return (
        <div className="fixed inset-y-0 right-0 z-50 w-full max-w-[440px] bg-white shadow-2xl flex flex-col border-l border-slate-200">
            ...
            <header className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-slate-900 text-lg font-bold leading-tight">NoteMind AI</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                        ...
                        {msg.role === "ai" ? (
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex flex-col gap-2 max-w-[85%]">
                                    <p className="text-slate-900 text-sm font-medium ml-1">NoteMind AI</p>
                                    <div className="bg-slate-100 rounded-2xl rounded-tl-none px-4 py-3">
                                        <p className="text-slate-700 text-[15px] leading-relaxed">{msg.text}</p>
                                    </div>
                                    {/* Related notes */}
                                        ...
                                        <div className="flex flex-col gap-2 mt-1">
                                            {msg.relatedNotes.map((note) => (
                                                <div
                                                    ...
                                                    className="group cursor-pointer flex items-center gap-3 rounded-xl bg-white border border-slate-200 p-3 hover:border-primary transition-all shadow-sm"
                                                >
                                                    <div className="bg-primary/5 rounded-lg size-10 flex items-center justify-center shrink-0">
                                                        <span className="material-symbols-outlined text-primary text-base">description</span>
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-slate-900 text-sm font-semibold truncate">{note.title}</p>
                                                        <p className="text-slate-500 text-xs truncate">{note.summary || "No summary"}</p>
                                                    </div>
                            ...
                            <div className="flex items-start gap-3 flex-row-reverse">
                                ...
                                <div className="flex flex-col gap-1 items-end max-w-[85%]">
                                    <p className="text-slate-900 text-sm font-medium mr-1">You</p>
                ...
                {loading && (
                    <div className="flex items-start gap-3">
                        ...
                        <div className="bg-slate-100 rounded-2xl rounded-tl-none px-4 py-3 flex gap-1 items-center">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {suggestions.map((s, i) => (
                            <button
                                ...
                                className="px-3 py-1 rounded-full border border-slate-200 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors"
                            >
                                {s}
                            </button>
                        ))}
                    ...
                    <div className="relative">
                        <textarea
                            ...
                            rows={1}
                            className="w-full pl-4 pr-12 py-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-primary/50 text-slate-900 resize-none placeholder:text-slate-400 outline-none"
                        />
                        <button
                            onClick={() => handleSend()}
